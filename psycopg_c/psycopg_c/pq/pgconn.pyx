...
cdef class PGconn:
    ...
    def send_sync_message(self) -> None:
        """Mark a synchronization point in a pipeline but do not flush the output buffer.

        :raises ~e.OperationalError: if the flush request failed.
        """
        if libpq.PG_VERSION_NUM < 160000:
            raise e.NotSupportedError(
                f"PQsendSyncMessage requires libpq from PostgreSQL 16,"
                f" {libpq.PG_VERSION_NUM} available instead"
            )
        cdef int rv = libpq.PQsendSyncMessage(self._pgconn_ptr)
        if rv == 0:
            raise e.OperationalError("connection not in pipeline mode")
        if rv != 1:
            raise e.OperationalError("failed to sync pipeline")
